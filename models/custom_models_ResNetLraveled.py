...
class AsymResLNet10F(nn.Module):
    def __init__(self, image_channels=3, n_classes = 10, base_channels=64, algorithm='FA', normalization='BatchNorm2d'): 
        super(AsymResLNet10F, self).__init__()
        self.n_classes = n_classes 
        self.base_channels = base_channels
        self.conv1 = Conv2d(image_channels, self.base_channels, kernel_size=7, stride=2, padding=3, bias=False, algorithm=algorithm)
...
class AsymResLNet10B(nn.Module):
    def __init__(self, image_channels=3, n_classes=10, algorithm='FA',base_channels=64, normalization='BatchNorm2d'):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.bn1 = getattr(nn, normalization)(self.base_channels, momentum=0.1,track_running_stats=False)
        self.conv1 = ConvTranspose2d(self.base_channels, image_channels , kernel_size=7, stride=2, padding=2, bias=False, output_padding=1, algorithm=algorithm) #output_padding=1
...
class AsymResLNet14F(nn.Module):
    def __init__(self, image_channels=3, n_classes = 10, base_channels=64, algorithm='FA', normalization='BatchNorm2d'): 
        ...
        self.bn41 = getattr(nn, normalization)(self.base_channels*2, momentum=0.1,track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)
        self.conv42 = Conv2d(self.base_channels*2, self.base_channels*4, kernel_size=3, stride=1, padding=1, algorithm=algorithm)
        self.bn42 = getattr(nn, normalization)(self.base_channels*4, momentum=0.1, track_running_stats=False)
        self.downsample2 =  Conv2d(self.base_channels*2, self.base_channels*4,kernel_size=1, stride=1, padding=0, algorithm=algorithm)
        self.bn43 = getattr(nn, normalization)(self.base_channels*4,momentum=0.1, track_running_stats=False)


        # layer 3
        self.conv51 = Conv2d(self.base_channels*4, self.base_channels*4, kernel_size=3, stride=2, groups=1, padding=1, algorithm=algorithm)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv62 = Conv2d(self.base_channels*4, self.n_classes, kernel_size=3, stride=1, padding=1, algorithm=algorithm)
...
class AsymResLNet14B(nn.Module):
    def __init__(self, image_channels=3, n_classes=10, algorithm='FA',base_channels=64, normalization='BatchNorm2d'):
        ...
        self.n_classes = n_classes
         ...
         # layer 3
        # self.bn43 = nn.BatchNorm2d(self.base_channels*4,momentum=0.1, track_running_stats=False)
        self.upsample3 =  ConvTranspose2d(self.n_classes, self.base_channels*4,kernel_size=1, stride=2, padding=0, output_padding=1, algorithm=algorithm)
        self.bn62 = getattr(nn, normalization)(self.n_classes,momentum=0.1, track_running_stats=False)
        self.conv62 = ConvTranspose2d(self.n_classes, self.base_channels*4, kernel_size=3,stride=1,  padding=1, output_padding=0, algorithm=algorithm)
        self.relu = nn.ReLU(inplace=True)
        self.bn61 = getattr(nn, normalization)(self.base_channels*4,momentum=0.1, track_running_stats=False)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.bn51 = getattr(nn, normalization)(self.base_channels*4,momentum=0.1, track_running_stats=False)
        self.conv51 = ConvTranspose2d(self.base_channels*4, self.base_channels*4, kernel_size=3, stride=2, groups=1, padding=1, output_padding=1, algorithm=algorithm) #output_padding=1


        # layer 2
        self.bn43 = getattr(nn, normalization)(self.base_channels*4,momentum=0.1, track_running_stats=False)
        self.upsample2 =  ConvTranspose2d(self.base_channels*4, self.base_channels*2,kernel_size=1, stride=1, padding=0,output_padding=0, algorithm=algorithm)
        self.bn42 = getattr(nn, normalization)(self.base_channels*4,momentum=0.1, track_running_stats=False)
        self.conv42 = ConvTranspose2d(self.base_channels*4, self.base_channels*2, kernel_size=3, stride=1, padding=1, output_padding=0, algorithm=algorithm)
        self.relu = nn.ReLU(inplace=True)
        self.bn41 = getattr(nn, normalization)(self.base_channels*2, momentum=0.1,track_running_stats=False)
        self.conv41 = ConvTranspose2d(self.base_channels*2, self.base_channels*2, stride=1, kernel_size=3, padding=1, output_padding=0, algorithm=algorithm)
        self.bn32 = getattr(nn, normalization)(self.base_channels*2, momentum=0.1, track_running_stats=False)
