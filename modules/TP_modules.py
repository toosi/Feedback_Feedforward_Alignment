...
class LinearFunction(autograd.Function):
    ...
    def backward(context, grad_output):
        ...
        if context.needs_input_grad[1]:
            ...
            grad_weight = grad_output.t().mm(input) # update for forward
        
        if context.needs_input_grad[2]:
            # only YY and TP needs gradients for backward weights
            if context.algorithm == 'TP':
                grad_weight_feedback = (input - input.mm(weight).mm(weight_feedback)).t().mm(input)
            else:
                # even YY does not need here because it is not layerwise
                grad_weight_feedback = None # grad_output.t().mm(input)  #  update for backward
